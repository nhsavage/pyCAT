...
#       start = dt.datetime(args.start_year, 1, 1)
        t_start = cftime._cftime.DatetimeNoLeap(args.start_year, 1, 1)
        constraint &= iris.Constraint(time=lambda cell: t_start <= cell.point)
    except:
        logging.debug('No start year given. Take it from input')
    try:
#       end = dt.datetime(args.end_year, 1, 1)
        t_end = cftime._cftime.DatetimeNoLeap(args.end_year, 1, 1)
        constraint &= iris.Constraint(time=lambda cell: cell.point < t_end)
    except:
...
# In Python 3, start_date and end_date are cftime objects, exact type depends on the calendar
    start_date, end_date = \
        time.units.num2date([time.points[0], time.points[-1]])

    try:
        if t_start.year < start_date.year:
    ...
    for year in range(start_date.year, end_date.year + 1):
        ...
        iris.fileformats.netcdf.save(cube.extract(constraint), fn, netcdf_format='NETCDF4', unlimited_dimensions='time',
            least_significant_digit=1, packing='short')
